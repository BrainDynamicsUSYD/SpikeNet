...
void Neurons::random_V(double p){
	...
	if ( p > 0.0 && p <= 1.0){
		...
		//boost::binomial_distribution<int> binml_spike(2,3.0);
...
void Neurons::add_neuron_sampling(vector<int> neuron_sample_ind_input, vector<bool> neuron_sample_type_input){
	...
	for (int c = 0; c < data_type_tot; ++c){
		if (neuron_sample_type[c]){
			...
			for (int i = 0; i < sample_size; ++i){ neuron_sample[c][i].resize(step_tot); }
		}
	}
}

void Neurons::add_pop_sampling(vector<bool> pop_sample_ind_input){
	...
	pop_sample.reserve(count*N); // "reserve" sets the capacity of the container vector
}
...
void Neurons::sample_data(int step_current){
	if (!neuron_sample_ind.empty()){
		for (unsigned int i = 0; i < neuron_sample_ind.size(); ++i){
			...
			if (neuron_sample_type[0]){neuron_sample[0][i][step_current] = V[ind_temp];}
			if (neuron_sample_type[1]){neuron_sample[1][i][step_current] = I_leak[ind_temp];}
			if (neuron_sample_type[2]){neuron_sample[2][i][step_current] = I_AMPA[ind_temp];}
			if (neuron_sample_type[3]){neuron_sample[3][i][step_current] = I_GABA[ind_temp];}
			if (neuron_sample_type[4]){neuron_sample[4][i][step_current] = I_NMDA[ind_temp];}
			if (neuron_sample_type[5]){neuron_sample[5][i][step_current] = I_GJ[ind_temp];}
			if (neuron_sample_type[6]){neuron_sample[6][i][step_current] = I_ext[ind_temp];}
		}
	}

	if (!pop_sample_ind.empty()){
		if (pop_sample_ind[step_current]){
			pop_sample.push_back(V);
		}
	}
}
...
void Neurons::output_results(ofstream& output_file, char delim, char indicator){
	...
	if (!pop_sample_ind.empty()){
		...
		output_file << pop_ind << delim << pop_sample.size() << delim  <<  endl;
		write2file(output_file, delim, pop_sample); // 2D matrix
	}
	...
	if (!neuron_sample_ind.empty()){
		...
		for (unsigned int c = 0; c < neuron_sample_type.size(); ++c){
			if (!neuron_sample[c].empty()){
				write2file(output_file, delim, neuron_sample[c]); // I_sample[.] is a 2D matrix
			}
		}
	}
