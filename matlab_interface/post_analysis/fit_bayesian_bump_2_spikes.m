function [ x, y, width, mlh, height  ] = fit_bayesian_bump_2_spikes( spike_x_pos_o,spike_y_pos_o, fw, mode)
...
if isempty(spike_x_pos_o)
    ...
    for i = i_pbc
        ...
        switch lower(mode)
            case 'bayesian'
                [v, fval] = fminsearchbnd(func_handle, x0, [min(x_r) min(y_r) 0 0] , [max(x_r) max(y_r)  500 100*fw]);
                mlh = -fval;
            case 'quick'
                v = x0;
                mlh = 0;
