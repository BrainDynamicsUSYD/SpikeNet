...
for r_num = 1:Result_num
    ...
    % Get Firing rate of spontaneous and evoked activity
    Result_cell{r_num} = get_rate_spon_evoked(Result_cell{r_num});
    ...
    % transfer spike history as the format of spikegrid
    try
        Result_cell{r_num} = SpikeNet2SpikeGrid_spikehist(Result_cell{r_num});
    catch
        warning('The ratio of EI neurons is not 3:1!');
    end
    
    
    % Generate comments
    Result_cell{r_num} = generate_commentsGC_ELIF(Result_cell{r_num});
    % calcuate susceptibility
    [ Result_cell{r_num} ] = get_susceptibility(Result_cell{r_num});
