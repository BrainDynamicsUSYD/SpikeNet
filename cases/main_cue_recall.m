function main_cue_recall(varargin)
...
for noise_k = 0.9:0.1:1.6
    for cue_k = 0.1:0.1:0.4
        
        for lesion_left = 0.5 %1.1:0.1:1.4 % range [0-1]
            % for P_random_control = 0:0.01:0.2
            for EE_factor = 0.6 % 0.8:0.1:1.2;
                for II_factor = 0.8 % 0.8:0.1:1.2;
                        ...
                        for rr = [0.6]
                            for Mnum = 8 %!!!
                                ...
                                for rate_ext = 4.4*ones(1,50) %linspace(4.0,4.0,45) %4.0:0.025:4.5 %4.1:0.025:4.5; % Hz
                                    ...
                                    Num_ext = 400;
                                    K_ext = Kmat(1,1);
                                    rate_t = ones(1,step_tot)*rate_ext;
                                    
                                    
                                    rate_noise = rate_t * noise_k;
                                    
                                    writeExtSpikeSettings(FID, 1, 1, K_ext,  Num_ext, rate_noise, 1, N(1));
                                    writeExtSpikeSettings(FID, 2, 1, K_ext,  Num_ext, rate_noise, 1, N(2));
                                    
                                    rate_cue = rate_t * cue_k;
                                    rate_cue(1:sec*0.5) = 0;
                                    writeExtSpikeSettings(FID, 1, 1, K_ext,  Num_ext, rate_cue, 1, N(1)/Mnum); % only to the first cluster
